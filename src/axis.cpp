...
ra_axis_c::ra_axis_c(byte imotorport, int encoder_PPR, int igearTeeth, EEPROMHandler_c* EEPROMObj) {
	...
	motor = new SFMotor(motorport, MOTOR12_1KHZ);
...
void ra_axis_c::stopSlew() {
	Serial.print("slewingEast || slewingWest: ");
	Serial.println(slewingEast || slewingWest);
	if(slewingEast || slewingWest) {
		slewSpeed = 0;
		// unset slewing flags
		slewingEast = false;
...
void dec_axis_c::stopSlew() {
	Serial.print(": slewingNorth || slewingSouth: ");
	Serial.println(slewingNorth || slewingSouth);
	if(slewingNorth || slewingSouth) {
		slewSpeed = 0;
		// unset slewing flags
		slewingNorth = false;
